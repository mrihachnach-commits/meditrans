...
export default function App() {
      ...
      if (currentUser) {
        ...
        if (!userSnap.exists()) {
          const userData = {
            ...
            displayName: currentUser.displayName || (currentUser.email === "admin@gmail.com" ? "Administrator" : ""),
          ...
          // Auto-upgrade to admin if email matches but role was 'user'
  ...
  const [settingsTab, setSettingsTab] = useState<'api' | 'admin'>('api');
  const [allUsers, setAllUsers] = useState<any[]>([]);

  useEffect(() => {
    if (isAdmin && showSettings && settingsTab === 'admin') {
  ...
  }, [isAdmin, showSettings, settingsTab]);
              ...
              <div className="flex flex-col h-[600px] max-h-[90vh]">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-display font-bold text-slate-800">Cài đặt hệ thống</h3>
                  </div>
                  <button 
                    onClick={() => setShowSettings(false)}
                ...
                <div className="flex flex-1 overflow-hidden flex-col sm:flex-row">
                  {/* Sidebar Tabs */}
                  <div className="w-full sm:w-48 bg-slate-50 border-b sm:border-b-0 sm:border-r border-slate-100 p-2 sm:p-4 flex sm:flex-col gap-2 shrink-0 overflow-x-auto no-scrollbar">
                    <button 
                      onClick={() => setSettingsTab('api')}
                      className={cn(
                        "flex-1 sm:w-full flex items-center justify-center sm:justify-start gap-3 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all whitespace-nowrap",
                        settingsTab === 'api' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:bg-slate-100"
                      )}
                    >
                    ...
                    {isAdmin && (
                      <button 
                        ...
                        className={cn(
                          "flex-1 sm:w-full flex items-center justify-center sm:justify-start gap-3 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all whitespace-nowrap",
                          settingsTab === 'admin' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:bg-slate-100"
                        )}
                      >
                    ...
                    {settingsTab === 'api' ? (
                        ...
                        <div>
                          ...
                          {(window as any).aistudio?.openSelectKey && (
                            <div className="mt-6">
                              ...
                              >
                                <Key className="w-4 h-4" />
                                Chọn API Key từ hệ thống
                              </button>
                              <p className="mt-2 text-[10px] text-slate-400 text-center">
                                Sử dụng API Key trả phí để có tốc độ dịch nhanh hơn và không bị giới hạn.
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                          <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Danh sách người dùng</h4>
                          <span className="px-2 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-full">
                          ...
                          {allUsers.map((u) => (
                            <div key={u.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-bold text-slate-800">{u.displayName || 'No Name'}</p>
                              ...
                              <div className="flex items-center gap-2">
                                <span className={cn(
                                  ...
                                  u.role === 'admin' ? "bg-amber-100 text-amber-600" : "bg-slate-200 text-slate-600"
                                )}>
                                  {u.role}
                                </span>
                                {u.email !== user?.email && (
                                  <button 
                                    onClick={async () => {
                                      if (confirm(`Bạn có chắc muốn đổi vai trò của ${u.email}?`)) {
                        ...
                        <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
                          <div className="flex gap-3">
                            ...
                            <div>
                              <p className="text-xs font-bold text-amber-800 mb-1">Khu vực quản trị</p>
                              <p className="text-[10px] text-amber-700 leading-relaxed">
                                Bạn có thể thay đổi vai trò của người dùng. Tài khoản admin có quyền truy cập tất cả tài liệu và cấu hình hệ thống.
                              </p>
                            </div>
                          </div>
                        </div>
